...
def test_pre_processing(get_ometiff_data_dir, setup_data_save_folder):
    ...
    # Check Stokes
    assert (
        np.sum(np.abs(stokes_denoise[0, z, :, :] - array[0, 0, z]) ** 2)
        / np.sum(np.abs(stokes_denoise[0, z, :, :])) ** 2
        < 0.1
    )
    assert (
        np.sum(np.abs(stokes_denoise[1, z, :, :] - array[0, 1, z]) ** 2)
        / np.sum(np.abs(stokes_denoise[1, z, :, :])) ** 2
        < 0.1
    )
    assert (
        np.sum(np.abs(stokes_denoise[2, z, :, :] - array[0, 2, z]) ** 2)
        / np.sum(np.abs(stokes_denoise[2, z, :, :])) ** 2
        < 0.1
    )
    assert (
        np.sum(np.abs(stokes_denoise[3, z, :, :] - array[0, 3, z]) ** 2)
        / np.sum(np.abs(stokes_denoise[3, z, :, :])) ** 2
        < 0.1
    )
...
def test_post_processing(get_ometiff_data_dir, setup_data_save_folder):
    ...
    path_to_config = os.path.join(
        dirname(dirname(abspath(__file__))),
        "test_configs/config_postprocessing_pytest.yml",
    )
    config = ConfigReader(
        path_to_config, data_dir=ometiff_data, save_dir=save_folder
    )
    ...
    fluor3D = deconvolve_fluorescence_3D(
        data_decon, manager.deconv_reconstructor, bg_level, reg=[1e-4, 1e-4]
    )

    # Check Birefringence
    assert (
        np.sum(np.abs(ret_denoise[z] - array[0, 0, z]) ** 2)
        / np.sum(np.abs(ret_denoise[z]))
        < 0.1
    )

    # Check deconvolution
    assert (
        np.sum(np.abs(array[0, 2, z] - fluor3D[1, z]) ** 2)
        / np.sum(np.abs(array[0, 2, z]) ** 2)
        < 0.1
    )
    ...
    assert (
        np.sum(
            np.abs(array[0, 1, z, 100:, 100:] - fluor3D[0, z, 0:-100, 0:-100])
            ** 2
        )
