...
def test_3D_reconstruction(get_zarr_data_dir, setup_data_save_folder):
    ...
    fluor3D = deconvolve_fluorescence_3D(
        data[t, 0], recon, bg_level, [config.reg]
    )
    # fluor3D = np.transpose(fluor3D, (-1, -3, -2))

    store = zarr.open(
        os.path.join(save_folder, "2T_3P_16Z_128Y_256X_Kazansky.zarr"), "r"
    )
    ...
    assert (
        np.sum(np.abs(fluor3D[z] - array[0, 0, z]) ** 2)
        / np.sum(np.abs(fluor3D[z]) ** 2)
        < 0.1
    )
...
def test_2D_reconstruction(get_zarr_data_dir, setup_data_save_folder):
    ...
    fluor2D = deconvolve_fluorescence_2D(
        data, recon, bg_level, reg=[config.reg]
    )
    store = zarr.open(
        os.path.join(save_folder, "2T_3P_16Z_128Y_256X_Kazansky.zarr"), "r"
    )
    ...
    # Check Deconvolved Fluor
    assert (
        np.sum(np.abs(fluor2D - array[0, 0, 0]) ** 2)
        / np.sum(np.abs(fluor2D) ** 2)
        < 0.1
    )


def test_deconvolution_and_registration(
    get_zarr_data_dir, setup_data_save_folder
):
    ...
    fluor3D = deconvolve_fluorescence_3D(
        data_decon, recon, bg_level, reg=[config.reg] * 2
    )
    ...
    assert (
        np.sum(
            np.abs(array[0, 0, z, 100:, 100:] - fluor3D[0, z, 0:-100, 0:-100])
            ** 2
        )
        / np.sum(np.abs(array[0, 0, z, 100:, 100:]) ** 2)
        < 0.1
    )
    ...
    assert (
        np.sum(
            np.abs(array[0, 1, z, 100:, 100:] - fluor3D[1, z, 0:-100, 0:-100])
            ** 2
        )
        / np.sum(np.abs(array[0, 1, z, 100:, 100:]) ** 2)
        < 0.1
    )
    ...
    assert (
        np.sum(
            np.abs(array[0, 2, z, 100:, 100:] - data[t, 2, z, 0:-100, 0:-100])
            ** 2
        )
        / np.sum(np.abs(array[0, 2, z, 100:, 100:]) ** 2)
        < 0.1
    )
