...
def test_pipeline_manager_run(init_phase_bf_pipeline_manager):
    ...
    store = zarr.open(
        os.path.join(save_folder, "2T_3P_16Z_128Y_256X_Kazansky_BF_1.zarr")
    )
...
def test_3D_reconstruction(get_bf_data_dir, setup_data_save_folder):
    ...
    path_to_config = os.path.join(
        ...
        "test_configs/phase/config_phase_3D_pytest.yml",
    )
    config = ConfigReader(
        path_to_config, data_dir=bf_data, save_dir=save_folder
    )
    ...
    # Check Phase
    assert (
        np.sum(np.abs(phase3D[z] - array[0, 0, z]) ** 2)
...
def test_2D_reconstruction(get_bf_data_dir, setup_data_save_folder):
    ...
    path_to_config = os.path.join(
        dirname(dirname(abspath(__file__))),
        "test_configs/phase/config_phase_2D_pytest.yml",
    )
    config = ConfigReader(
        path_to_config, data_dir=bf_data, save_dir=save_folder
    )
    ...
    # Check Phase
    assert (
        np.sum(np.abs(phase2D - array[0, 0, 0]) ** 2)
