...
def test_pipeline_manager_run(init_qlipp_tiff_pipeline_manager):
    ...
    store = zarr.open(
        os.path.join(save_folder, "2T_3P_16Z_128Y_256X_Kazansky_1.zarr"), "r"
    )
...
def test_3D_reconstruction(get_ometiff_data_dir, setup_data_save_folder):
    ...
    path_to_config = os.path.join(
        ...
        "test_configs/qlipp/config_qlipp_full_recon_pytest_tiff.yml",
    )
    config = ConfigReader(
        path_to_config, data_dir=ometiff_data, save_dir=save_folder
    )
...
def test_2D_reconstruction(get_ometiff_data_dir, setup_data_save_folder):
    ...
    path_to_config = os.path.join(
        dirname(dirname(abspath(__file__))),
        "test_configs/qlipp/config_qlipp_2D_pytest_tiff.yml",
    )
    config = ConfigReader(
        path_to_config, data_dir=ometiff_data, save_dir=save_folder
    )
