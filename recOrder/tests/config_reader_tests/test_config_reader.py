...
def test_config_reader(get_zarr_data_dir):
    ...
    path_to_config = os.path.join(
        ...
        "test_configs/qlipp/config_qlipp_full_pytest.yml",
    )
    ...
    for key, value in PREPROCESSING.items():
        for sub_key, sub_value in PREPROCESSING[key].items():
            assert raw["pre_processing"][key][sub_key] == getattr(
                config.preprocessing, f"{key}_{sub_key}"
            )
    ...
    for key, value in POSTPROCESSING.items():
        for sub_key, sub_value in POSTPROCESSING[key].items():
            assert raw["post_processing"][key][sub_key] == getattr(
                config.postprocessing, f"{key}_{sub_key}"
            )
