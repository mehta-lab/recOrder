...
POSTPROCESSING = {
    ...
    "registration": {"use": False, "channel_idx": None, "shift": None},
}
...
class ConfigReader(object):
        ...
        for key, value in PREPROCESSING.items():
            if isinstance(value, dict):
                for key_child, value_child in PREPROCESSING[key].items():
                    setattr(
                        self.preprocessing, f"{key}_{key_child}", value_child
                    )
            else:
        ...
        for key, value in POSTPROCESSING.items():
            if isinstance(value, dict):
                for key_child, value_child in POSTPROCESSING[key].items():
                    setattr(
                        self.postprocessing, f"{key}_{key_child}", value_child
                    )
            else:
    ...
    def _check_assertions(self, data_dir, save_dir, method, mode, name):
        ...
        if not method:
            assert (
                "method" in self.config["dataset"]
            ), "Please provide method in config file or CLI argument"
        if not mode:
            assert (
                "mode" in self.config["dataset"]
            ), "Please provide mode in config file or CLI argument"
        if not data_dir:
            assert (
                "data_dir" in self.config["dataset"]
            ), "Please provide data_dir in config file or CLI argument"
        if not save_dir:
            assert (
                "save_dir" in self.config["dataset"]
            ), "Please provide save_dir in config file or CLI argument"

        if self.config["dataset"]["positions"] != "all" and not isinstance(
            self.config["dataset"]["positions"], int
        ):
            assert isinstance(
                self.config["dataset"]["positions"], list
            ), 'if not single integer value or "all", positions must be list (nested lists/tuples allowed)'
        if self.config["dataset"]["timepoints"] != "all" and not isinstance(
            self.config["dataset"]["timepoints"], int
        ):
            assert isinstance(
                self.config["dataset"]["timepoints"], list
            ), 'if not single integer value or "all", timepoints must be list (nested lists/tuples allowed)'

        for key, value in PROCESSING.items():
            if key == "output_channels":
                if (
                    "Phase3D" in self.config["processing"][key]
                    or "Phase2D" in self.config["processing"][key]
                ):
                    assert (
                        "wavelength" in self.config["processing"]
                    ), "wavelength required for phase reconstruction"
                    assert (
                        "pixel_size" in self.config["processing"]
                    ), "pixel_size required for phase reconstruction"
                    assert (
                        "magnification" in self.config["processing"]
                    ), "magnification required for phase reconstruction"
                    assert (
                        "NA_objective" in self.config["processing"]
                    ), "NA_objective required for phase reconstruction"
                    assert (
                        "NA_condenser" in self.config["processing"]
                    ), "NA_condenser required for phase reconstruction"

                if (
                    "Phase3D" in self.config["processing"][key]
                    and "Phase2D" in self.config["processing"][key]
                ):
                    raise KeyError(
                        f"Both Phase3D and Phase2D cannot be specified in {key}.  Please compute separately"
                    )

                if (
                    "Phase3D" in self.config["processing"][key]
                    and self.config["dataset"]["mode"] == "2D"
                ):
                    raise KeyError(
                ...
                if (
                    "Phase2D" in self.config["processing"][key]
                    and self.config["dataset"]["mode"] == "3D"
                ):
                    raise KeyError(
                ...
                key == "background_correction"
                and self.config["dataset"]["method"] == "QLIPP"
            ):
                if (
                    self.config["processing"][key] == "None"
                    or self.config["processing"][key] == "local_fit"
                ):
                    pass
                else:
                    assert (
                        self.config["dataset"]["background"] is not None
        ...
        if "preprocessing" in self.config:
            for key, value in PREPROCESSING.items():
                if self.config["preprocessing"][key]["use"]:
                    for key_child, value_child in PREPROCESSING[key].items():
                        assert (
                            key_child in self.config["preprocessing"][key]
        ...
        if "postprocessing" in self.config:
            for key, value in POSTPROCESSING.items():
                if self.config["postprocessing"][key]["use"]:
                    for key_child, value_child in POSTPROCESSING[key].items():
                        assert (
                            key_child in self.config["postprocessing"][key]
                        ), f"User must specify {key_child} to use for {key}"
    ...
    def _create_yaml_dict(self):
        ...
        for key, value in PREPROCESSING.items():
            if isinstance(value, dict):
                ...
                for key_child, value_child in PREPROCESSING[key].items():
                    yaml_dict["pre_processing"][key][key_child] = getattr(
                        self.preprocessing, f"{key}_{key_child}"
                    )
            else:
        ...
        for key, value in POSTPROCESSING.items():
            if isinstance(value, dict):
                ...
                for key_child, value_child in POSTPROCESSING[key].items():
                    yaml_dict["post_processing"][key][key_child] = getattr(
                        self.postprocessing, f"{key}_{key_child}"
                    )
            else:
    ...
    def _parse_dataset(self):
        for key, value in self.config["dataset"].items():
            if key in DATASET.keys():
                ...
                # this section appends the rest of the dataset parameters specified
                    ...
                    if isinstance(value, str):
                        if value == "all":
                            ...
                            raise KeyError(
                                f'{key} value {value} not understood,\
                        ...
                        if len(value) == 2:
                            ...
                            raise KeyError(
                                f"{key} value {value} is not a tuple with length of 2"
                            )
                    elif isinstance(value, list):
                        super().__setattr__(key, value)
                    else:
                        raise KeyError(
                            f'{key} value {value} format not understood. \
                ...
                warnings.warn(
                    f"yaml DATASET config field {key} is not recognized"
                )
        ...
        if "pre_processing" in self.config:
            for key, value in self.config["pre_processing"].items():
                if key in PREPROCESSING.keys():
                    for key_child, value_child in self.config[
                        "pre_processing"
                    ][key].items():
                        if key_child in PREPROCESSING[key].keys():
                            ...
                            warnings.warn(
                                f"yaml PREPROCESSING config field {key}, {key_child} is not recognized"
                            )
                else:
                    warnings.warn(
                        f"yaml PREPROCESSING config field {key} is not recognized"
                    )
    ...
    def _parse_processing(self):
        for key, value in self.config["processing"].items():
            if key in PROCESSING.keys():
                ...
                warnings.warn(
                    f"yaml PROCESSING config field {key} is not recognized"
                )

        if (
            "Phase3D" not in self.output_channels
            and "Phase2D" not in self.output_channels
        ):
                ...
                if key in POSTPROCESSING.keys():
                    for key_child, value_child in self.config[
                        ...
                        if key_child in POSTPROCESSING[key].keys():
                            ...
                            warnings.warn(
                                f"yaml POSTPROCESSING config field {key}, {key_child} is not recognized"
                            )
                else:
                    warnings.warn(
                        f"yaml POSTPROCESSING config field {key} is not recognized"
                    )
