...
class QLIPP(PipelineInterface):
    ...
    def __init__(
        ...
        if self.data.channels < 4:
            raise ValueError(
                f"Number of Channels is {data.channels}, cannot be less than 4"
            )
        ...
        self.bg_path = (
            self.config.background if self.config.background else None
        )
        ...
        (
            ...
            self.s4_idx,
            self.fluor_idxs,
        ) = self.parse_channel_idx(self.data.channel_names)
        ...
        wo_background_correction = rec_bkg_to_wo_bkg(
            self.config.background_correction
        )

        # Initialize Reconstructor
        if self.no_phase:
            self.reconstructor = initialize_reconstructor(
                pipeline="birefringence",
        ...
        if self.config.background_correction in ["global", "local_fit+"]:
            bg_data = load_bg(
                self.bg_path, self.img_dim[0], self.img_dim[1], self.bg_roi
            )  # TODO: remove ROI for 1.0.0
            self.bg_stokes = self.reconstructor.Stokes_recon(bg_data)
            self.bg_stokes = self.reconstructor.Stokes_transform(
                self.bg_stokes
            )
    ...
    def _check_output_channels(self, output_channels):
        ...
        for channel in output_channels:
            if (
                "Retardance" in channel
                or "Orientation" in channel
                or "Brightfield" in channel
            ):
    ...
    def reconstruct_stokes_volume(self, data):
        ...
        if self.calib_scheme == "4-State":
            LF_array = np.zeros(
                [4, self.data.slices, self.data.height, self.data.width]
            )
            ...
            LF_array = np.zeros(
                [5, self.data.slices, self.data.height, self.data.width]
            )
            LF_array[0] = data[self.s0_idx]
            LF_array[1] = data[self.s1_idx]
            LF_array[2] = data[self.s2_idx]
            ...
            raise NotImplementedError(
                f"calibration scheme {self.calib_scheme} not implemented"
            )

        stokes = reconstruct_qlipp_stokes(
            LF_array, self.reconstructor, self.bg_stokes
        )
    ...
    def reconstruct_birefringence_volume(self, stokes):
        ...
        if self.no_birefringence:
            ...
            return reconstruct_qlipp_birefringence(
                stokes[
                    :,
                    slice(None) if self.slices != 1 else self.focus_slice,
                    :,
                    :,
                ],
                self.reconstructor,
    ...
    def write_data(
        ...
        for chan in range(len(self.output_channels)):
            if "Retardance" in self.output_channels[chan]:
                ...
                self.writer.write(
                    stokes[0, slice_, :, :], p=p, t=t, c=chan, z=z
                )
            elif "S1" in self.output_channels[chan]:
                self.writer.write(
                    stokes[1, slice_, :, :], p=p, t=t, c=chan, z=z
                )
            elif "S2" in self.output_channels[chan]:
                self.writer.write(
                    stokes[2, slice_, :, :], p=p, t=t, c=chan, z=z
                )
            elif "S3" in self.output_channels[chan]:
                self.writer.write(
                    stokes[3, slice_, :, :], p=p, t=t, c=chan, z=z
                )
    ...
    def parse_channel_idx(self, channel_list):
        ...
        s4_idx = None
        if (
            "PolScope_Plugin_Version"
            in self.calib_meta.json_metadata["Summary"]
        ):
            open_pol = True
        else:
            open_pol = False
