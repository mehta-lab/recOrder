...
def create_empty_zarr(
    ...
    for path in position_paths:
        ...
        # TODO: this dtype should be based off of the output. Setting to float32 for now. EH
        _ = pos.create_zeros(
            name="0",
            shape=output_shape,
            chunks=chunk_size,
...
def apply_reconstruction_to_zyx_and_save(
    ...
    **kwargs,
    ...
    for input_channel_name in settings.input_channel_names:
        channel_indices.append(position.channel_names.index(input_channel_name))
    # Load data
    ...
    czyx_data = tczyx_data[t_idx, c_slice]
    # czyx_data = torch.tensor(position[0][t_idx, c_slice], dtype=torch.float32)

    # Apply transformation
    reconstruction_zyx = func(czyx_data, **kwargs)
    reconstruction_array = np.array([tensor.numpy() for tensor in reconstruction_zyx])
    if reconstruction_array.ndim == 3:
        reconstruction_array = np.expand_dims(reconstruction_array[slice(z_2D)], axis=0)
    # Write to file
...
def get_reconstruction_data_shape(position_paths: list[Path], config_path: Path):
    # Load the first position to infer dataset information
    input_dataset = open_ome_zarr(str(position_paths[0]), mode="r")
    T, C, Z, Y, X = input_dataset.data.shape
