...
@click.option(
    "--name", required=False, type=str, help="name to use for saving the data"
)
@click.option(
    "--config",
    required=True,
...
def reconstruct(
    method, mode, data_dir, save_dir, name, config, overwrite=False
):
    ...
    if config:
        if not os.path.exists(config):
            ...
            config = ConfigReader(
                config, data_dir, save_dir, method, mode, name, immutable=False
            )
            config.save_yaml()
    else:
        config = ConfigReader(
            None, data_dir, save_dir, method, mode, name, immutable=False
        )
        config.save_yaml()
