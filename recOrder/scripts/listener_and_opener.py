...
path = "/Applications/Micro-Manager-2.0.1-20220920/prac_folder/napari_test/test.zarr"
...
def mda_to_zarr(path):
    ...
    while datastore:
        ...
        required_coord = (
            intended_dimensions.copyBuilder().p(curr_p).t(curr_t).c(curr_c).z(curr_z).build()
        )
        found = False
        print(mode)
        if mode == "ND_TIFF":
