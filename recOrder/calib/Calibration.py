...
class QLIPP_Calibration:
    ...
    def check_and_get_roi(self):
        ...
        if len(boxes) == 0:
            raise ValueError(
                "No ROI Bounding Box Found, Please Draw Bounding Box on the Preview (live) Window"
            )

        if len(boxes) > 1:
            raise ValueError(
                "More than one Bounding Box Found, Please Remove any box not on the preview (live) window"
            )
    ...
    def display_and_check_ROI(self, rect):
        ...
        if cont in ["No", "N", "no", "n"]:
            ...
            raise ValueError(
                "Did not understand your answer, please check spelling"
            )
