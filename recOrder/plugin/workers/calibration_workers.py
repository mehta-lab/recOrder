...
class CalibrationWorker(WorkerBase):
    ...
    def work(self):
        ...
        self.calib.meta_file = os.path.join(
            self.calib_window.directory, "calibration_metadata.txt"
        )
        idx = 1
        while os.path.exists(self.calib.meta_file):
            if self.calib.meta_file == os.path.join(
                self.calib_window.directory, "calibration_metadata.txt"
            ):
                self.calib.meta_file = os.path.join(
                    self.calib_window.directory, "calibration_metadata_1.txt"
                )
            else:
                idx += 1
                self.calib.meta_file = os.path.join(
                    self.calib_window.directory,
                    f"calibration_metadata_{idx}.txt",
                )
...
class BackgroundCaptureWorker(WorkerBase):
    ...
    def work(self):
        ...
        self.calib.meta_file = os.path.join(
            bg_path, "calibration_metadata.txt"
        )

        microscope_params = {
            "n_objective_media": float(self.calib_window.ui.le_n_media.text())
            if self.calib_window.ui.le_n_media.text() != ""
            else None,
            "objective_NA": float(self.calib_window.ui.le_obj_na.text())
            if self.calib_window.ui.le_obj_na.text() != ""
            else None,
            "condenser_NA": float(self.calib_window.ui.le_cond_na.text())
            if self.calib_window.ui.le_cond_na.text() != ""
            else None,
            "magnification": float(self.calib_window.ui.le_mag.text())
            if self.calib_window.ui.le_mag.text() != ""
            else None,
            "pixel_size": float(self.calib_window.ui.le_ps.text())
            if self.calib_window.ui.le_ps.text() != ""
            else None,
        }
