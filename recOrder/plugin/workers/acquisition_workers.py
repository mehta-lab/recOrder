...
class FluorescenceAcquisitionWorker(WorkerBase):
    ...
    def __init__(self, calib_window):
        ...
        self.dim = (
            "2D"
            if self.calib_window.ui.cb_fluor_dim.currentIndex() == 0
            else "3D"
        )
    ...
    def _reconstruct(self, stack):
        ...
        self.img_dim = (
            (stack.shape[-2], stack.shape[-1], stack.shape[-3])
            if self.n_slices != 1
            else (stack.shape[-2], stack.shape[-1], 1)
        )
        ...
        if not self.calib_window.fluor_reconstructor:
            ...
            recon = initialize_fluorescence_reconstructor(
        ...
        if self.dim == "2D":
            ...
            fluor_deconvolved = deconvolve_fluorescence_2D(
                data=stack,
                reconstructor=recon,
                bg_level=bg_level,
            ...
            fluor_deconvolved = deconvolve_fluorescence_3D(
                data=stack,
                reconstructor=recon,
                bg_level=bg_level,
                reg=[float(self.calib_window.ui.le_fluor_strength.text())],
        ...
        meta["regularization_strength"] = float(
            self.calib_window.ui.le_fluor_strength.text()
        )
    ...
    def _save_imgs(self, fluor, meta=None):
        ...
        name = (
            f"FluorescenceDeconvolvedSnap.zarr"
            if not prefix
            else f"{prefix}_FluorescenceDeconvolvedSnap.zarr"
        )
        ...
        if len(fluor.shape) == 2:
            writer.init_array(
                0,
                (1, 1, 1, fluor.shape[-2], fluor.shape[-1]),
                chunk_size,
                ["FluorDeconvolved2D"],
            )
            z = 0
        ...
        # Check if 3D
            ...
            writer.init_array(
                0,
                (1, 1, fluor.shape[-3], fluor.shape[-2], fluor.shape[-1]),
                chunk_size,
                ["FluorDeconvolved3D"],
            )
    ...
    def _reconstructor_changed(self):
        ...
        if (recon.N, recon.M, recon.N_defocus) != self.img_dim:
            ...
            recon.lambda_emiss * recon.n_media
            != int(self.calib_window.ui.le_recon_wavelength.text()) / 1000
        ):
            changed = True
        ...
        for i in range(disps.size()):
            ...
            if self.prefix in disp.getName():
                ...
                try:
                    ...
                    name = (
                        f"RawFluorDataSnap.zarr"
                        if not save_prefix
                        else f"{save_prefix}_RawFluorDataSnap.zarr"
                    )
                    out_path = os.path.join(self.snap_dir, name)
                    converter = ZarrConverter(
                        os.path.join(dir_, prefix),
