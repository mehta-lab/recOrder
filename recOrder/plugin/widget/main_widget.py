...
from recOrder.io.config_reader import (
    ConfigReader,
    PROCESSING,
    PREPROCESSING,
    POSTPROCESSING,
...
class MainWidget(QWidget):
    ...
    def __init__(self, napari_viewer: Viewer):
        ...
        self.ui.qbutton_acq_fluor.clicked[bool].connect(
            self.acquire_fluor_deconvolved
        )
    ...
    def _check_requirements_for_acq(self, mode):
        ...
        if mode == "birefringence" or mode == "phase" or mode == "fluor":
            ...
            if (
                ...
                or self.bg_option == "local_fit+"
                or self.bg_option == "global"
            ):
    ...
    def _check_requirements_for_reconstruction(self):
        ...
        if self.method == "QLIPP":
            ...
            if qlipp_channel_present:
                ...
                if (
                    "Phase2D" in output_channels
                    or "Phase3D" in output_channels
                ):
                    for field in phase_required:
                        cont = self._check_line_edit(field)
                        tab = (
            ...
            tab = (
                getattr(self.ui, f"le_fluor_chan")
                .parent()
                .parent()
                .objectName()
            )
            ...
            if "Phase2D" in output_channels or "Phase3D" in output_channels:
                ...
                print(
                    "User did not specify any PhaseFromBF Specific Channels (Phase2D, Phase3D)"
                )
                success = False
    ...
    def _populate_config_from_app(self):
        ...
        self.config_reader.data_save_name = (
            self.ui.le_data_save_name.text()
            if self.ui.le_data_save_name.text() != ""
            else pathlib.PurePath(self.data_dir).name
        )
        self.config_reader.calibration_metadata = (
            self.ui.le_calibration_metadata.text()
        )
        ...
        for key, value in PREPROCESSING.items():
            if isinstance(value, dict):
                for key_child, value_child in PREPROCESSING[key].items():
                    if key_child == "use":
                        field = getattr(
                            self.ui, f"chb_preproc_{key}_{key_child}"
                        )
                        val = True if field.checkState() == 2 else False
                        setattr(
                            self.config_reader.preprocessing,
                            f"{key}_{key_child}",
                            val,
                        )
                    else:
                        field = getattr(
                            self.ui, f"le_preproc_{key}_{key_child}"
                        )
                        setattr(
                            self.config_reader.preprocessing,
                            f"{key}_{key_child}",
                ...
                setattr(
                    self.config_reader.preprocessing, key, getattr(self, key)
                )
            ...
            if key not in skip:
                if key == "background_correction":
                    ...
                    setattr(
                        self.config_reader,
                        key,
                        bg_map[self.ui.cb_bg_method.currentIndex()],
                    )
                    ...
                    if "Phase3D" in channels and self.mode == "2D":
                        ...
                        raise KeyError(
                            f"Specified mode is 2D and Phase3D was specified for reconstruction. "
                    ...
                    if "Phase2D" in channels and self.mode == "3D":
                        ...
                        raise KeyError(
                            f"Specified mode is 3D and Phase2D was specified for reconstruction. "
        ...
        if self.mode == "3D":
            if self.ui.cb_phase_denoiser.currentIndex() == 0:
                setattr(
                    self.config_reader,
                    "Tik_reg_ph_3D",
                    float(self.ui.le_phase_strength.text()),
                )
            else:
                setattr(
                    self.config_reader,
                ...
                setattr(
                    self.config_reader, "rho_3D", float(self.ui.le_rho.text())
                )
                setattr(
                    self.config_reader, "itr_3D", int(self.ui.le_itr.text())
                )
            ...
            if self.ui.cb_phase_denoiser.currentIndex() == 0:
                ...
                setattr(
                    self.config_reader, "rho_2D", float(self.ui.le_rho.text())
                )
                setattr(
                    self.config_reader, "itr_2D", int(self.ui.le_itr.text())
                )
        ...
        if self.method == "FluorDeconv":
            ...
            setattr(
                self.config_reader, "fluorescence_channel_indices", channels
            )
            ...
            if len(regs) == 1:
                ...
                if len(regs) != len(wavelengths):
                    ...
                    raise ValueError(
                        "Length of regularization values must match wavelengths"
                    )
                else:
                    self._set_tab_red("Regularization", False)
                    self.ui.le_recon_wavelength.setStyleSheet("")
            ...
            if self.ui.chb_autocalc_bg.isChecked():
                ...
                if len(bg) != len(channels):
                    ...
                    raise ValueError(
                        "Length of background values must match wavelengths"
                    )
                else:
                    self._set_tab_red("Regularization", False)
                    self.ui.le_recon_wavelength.setStyleSheet("")
            ...
            if len(wavelengths) != len(channels):
                ...
                raise ValueError(
                    "Wavelengths and output channels must be the same length"
                )
            ...
            if self.mode == "2D":
                ...
                if len(indices) != len(channels):
                    ...
                    raise ValueError(
                        "Wavelengths, Focused z-indices, and channels must be the same length"
                    )
                else:
                    self._set_tab_red("Processing", False)
                    self.ui.le_focus_zidx.setStyleSheet("")
            ...
            setattr(
                self.config_reader,
                "wavelength",
                int(self.ui.le_recon_wavelength.text()),
            )
            setattr(
                self.config_reader,
            ...
            setattr(
                self.config_reader,
                "focus_zidx",
                int(focus_zidx) if focus_zidx != "" else None,
            )
        ...
        setattr(
            ...
            if self.ui.le_obj_na.text() != ""
            else None,
        )
        setattr(
            self.config_reader,
            "NA_condenser",
            float(self.ui.le_cond_na.text())
            if self.ui.le_cond_na.text() != ""
            else None,
        )
        setattr(
            self.config_reader,
        ...
        setattr(
            ...
            if self.ui.le_n_media.text() != ""
            else None,
        )
        setattr(
            self.config_reader,
            "magnification",
            float(self.ui.le_mag.text())
            if self.ui.le_mag.text() != ""
            else None,
        )
    ...
    def _populate_from_config(self):
        ...
        self.ui.le_calibration_metadata.setText(
            self.config_reader.calibration_metadata
        )
        self.ui.le_bg_path.setText(self.config_reader.background)

        self.mode = self.config_reader.mode
        self.ui.cb_mode.setCurrentIndex(
            0
        ) if self.mode == "3D" else self.ui.cb_mode.setCurrentIndex(1)
        ...
        if isinstance(self.config_reader.timepoints, list):
            ...
            for idx, time in enumerate(timepoints):
                text += (
                    f"{time}, " if idx != len(timepoints) - 1 else f"{time}"
                )
        ...
        for key, val in PREPROCESSING.items():
            for key_child, val_child in val.items():
                if key_child == "use":
                    attr = getattr(
                        self.config_reader.preprocessing, "denoise_use"
                    )
        ...
        if not isinstance(wavelengths, list):
            self.ui.le_recon_wavelength.setText(
                str(int(self.config_reader.wavelength))
            )
        else:
            text = ""
            for idx, chan in enumerate(wavelengths):
                text += (
                    f"{chan}, " if idx != len(wavelengths) - 1 else f"{chan}"
                )
        ...
        if self.method == "PhaseFromBF":
            self.ui.le_fluor_chan.setText(
                str(self.config_reader.brightfield_channel_index)
            )
        if self.method == "FluorDeconv":
        ...
        for key, val in PROCESSING.items():
            if key == "output_channels":
                ...
                for idx, chan in enumerate(channels):
                    text += (
                        f"{chan}, " if idx != len(channels) - 1 else f"{chan}"
                    )
                ...
                if isinstance(indices, int):
                    ...
                    for idx, val in enumerate(indices):
                        text += (
                            f"{val}, " if idx != len(indices) - 1 else f"{val}"
                        )
                ...
                le.setText(
                    str(getattr(self.config_reader, key))
                    if not isinstance(getattr(self.config_reader, key), str)
                    else getattr(self.config_reader, key)
                )
                ...
                cfg_attr = getattr(
                    self.config_reader, f"phase_denoiser_{self.mode}"
                )
                denoiser = cfg_attr
                cb.setCurrentIndex(
                    0
                ) if cfg_attr == "Tikhonov" else cb.setCurrentIndex(1)
            else:
                if denoiser == "Tikhonov":
                    strength = getattr(
                        self.config_reader, f"Tik_reg_ph_{self.mode}"
                    )
                    self.ui.le_phase_strength.setText(str(strength))
                else:
                    strength = getattr(
                        self.config_reader, f"TV_reg_ph_{self.mode}"
                    )
                    self.ui.le_phase_strength.setText(str(strength))
                    self.ui.le_rho.setText(
                        str(getattr(self.config_reader, f"rho_{self.mode}"))
                    )
                    self.ui.le_itr.setText(
                        str(getattr(self.config_reader, f"itr_{self.mode}"))
                    )
        ...
        for key, val in POSTPROCESSING.items():
            for key_child, val_child in val.items():
                if key == "deconvolution":
                    if key_child == "use":
                        attr = getattr(
                            self.config_reader.postprocessing,
                            "registration_use",
                        )
                        self.ui.chb_preproc_denoise_use.setCheckState(attr)
                    ...
                    if key_child == "use":
                        attr = getattr(
                            self.config_reader.postprocessing, "denoise_use"
                        )
                        self.ui.chb_postproc_denoise_use.setCheckState(attr)
                    else:
                        le = getattr(
                            self.ui, f"le_postproc_denoise_{key_child}"
                        )
                        attr = str(
                            getattr(
                                self.config_reader.postprocessing,
    ...
    def enter_method(self):
        ...
        if idx == 0:
            ...
            self.ui.label_chan_desc.setText(
                "Retardance, Orientation, BF, Phase3D, Phase2D, Fluor1, "
            ...
            self.ui.label_chan_desc.setText(
                "Phase3D, Phase2D, Fluor1, Fluor2 (ex. DAPI, GFP)"
            )
    ...
    def load_calibration(self):
        ...
        if params is not None:
            self.ui.le_pad_z.setText(
                str(params["pad_z"]) if params["pad_z"] is not None else ""
            )
            self.ui.le_n_media.setText(
                str(params["n_objective_media"])
                if params["n_objective_media"] is not None
                else ""
            )
            self.ui.le_obj_na.setText(
                str(params["objective_NA"])
                if params["objective_NA"] is not None
                else ""
            )
            self.ui.le_cond_na.setText(
                str(params["condenser_NA"])
                if params["condenser_NA"] is not None
                else ""
            )
            self.ui.le_mag.setText(
                str(params["magnification"])
                if params["magnification"] is not None
                else ""
            )
            self.ui.le_ps.setText(
    ...
    def acquire_fluor_deconvolved(self):
        ...
        self.worker.meta_emitter.connect(self.handle_meta_update)
        self.worker.fluor_reconstructor_emitter.connect(
            self.handle_fluor_reconstructor_update
        )
        self.worker.started.connect(self._disable_buttons)
        self.worker.finished.connect(self._enable_buttons)
        self.worker.errored.connect(self._handle_acq_error)
