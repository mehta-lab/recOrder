...
def initialize_reconstructor(
    ...
    if pipeline == "QLIPP" or pipeline == "PhaseFromBF":
        ...
        if not mag:
            raise ValueError(
                "Please specify mag (magnification) in function parameters"
            )
        if not wavelength_nm:
            raise ValueError(
                "Please specify the wavelength for reconstruction"
            )
        if not n_slices:
        ...
        if pipeline == "QLIPP":
            if not calibration_scheme:
                raise ValueError(
                    "Please specify qlipp_scheme (calibration scheme) for QLIPP reconstruction"
                )
            if not swing:
        ...
        if not wavelength_nm:
            raise ValueError(
                "Please specify the wavelength for QLIPP reconstruction"
            )
        if not swing:
            raise ValueError("Please specify swing in function parameters")
        ...
        inst_mat = np.array(
            [
                ...
                [
                    ...
                    np.sqrt(3) * np.cos(np.pi * swing) * np.sin(np.pi * swing),
                    -np.cos(2 * np.pi * swing),
                ],
                [
                    1,
                    -0.5 * np.sin(2 * np.pi * swing),
                    -np.sqrt(3) / 2 * np.sin(2 * np.pi * swing),
                    -np.cos(2 * np.pi * swing),
                ],
            ]
        )
...
def reconstruct_phase2D(
    S0, recon, method="Tikhonov", reg_p=1e-4, rho=1, lambda_p=1e-4, itr=50
):
    """
    ...
    _, phase2D = recon.Phase_recon(
        np.copy(S0).astype("float"),
        method=method,
        reg_p=reg_p,
        rho=rho,
        lambda_p=lambda_p,
...
def reconstruct_phase3D(
    S0, recon, method="Tikhonov", reg_re=1e-4, rho=1e-3, lambda_re=1e-4, itr=50
):
    """
    Reconstruct 2D phase from a given S0 or BF stack.
...
class QLIPPBirefringenceCompute:
    ...
    def __init__(
        ...
        if bg_option != "None":
            self.bg_stokes = reconstruct_qlipp_stokes(
                bg_data, self.reconstructor
            )
        else:
            self.bg_stokes = None
    ...
    def reconstruct(self, array):
        ...
        stokes = reconstruct_qlipp_stokes(
            array, self.reconstructor, self.bg_stokes
        )
        birefringence = reconstruct_qlipp_birefringence(
            stokes, self.reconstructor
        )
